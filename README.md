...
- **Phase B — Test Documentation Generation.** Using the accumulated knowledge base, the agent generates per-module XLSX files containing structured test plans and test cases, traceable to requirements and non-duplicative of existing Qase entries. (Not yet activated.)
...
- Master system prompt (`CLAUDE+.md`, ~540 lines) defining identity, protocols, and quality standards
...
1. **Data Acquisition** (implemented, current mode). Autonomous multi-session investigation from scratch. The agent systematically builds a knowledge base by exploring the codebase (static analysis of local clone), live testing environments (Playwright UI, Swagger API, PostgreSQL), and external documentation (Confluence, Figma, GitLab, Qase). Sessions are orchestrated by `run-sessions.sh` with configurable delays and limits. Targets 80% knowledge coverage across all application areas before transitioning to generation. Current progress: ~12% after 1 session.
...
3. **Documentation Generation** (implemented, not yet activated). Enabled when Phase A coverage reaches the threshold. Produces per-module XLSX test plans and test cases using Python openpyxl. Each module goes through: knowledge sufficiency check → gap investigation if needed → Qase deduplication check → XLSX generation → tracking in SQLite. Output directory: `expert-system/output/`.
...
Phase A, early stage. Autonomous sessions execution is in progress:
- Infrastructure fully bootstrapped (vault, SQLite, MCP connections, repo clone)
- 21 knowledge base notes created covering architecture, modules, database schema, roles, absence data model, and external documentation
- Overall knowledge coverage: ~12% (target: 80%)
- Key areas mapped at overview level; deep business logic tracing, API-level documentation, UI flow exploration, and remaining Confluence/Figma/GitLab ingestion pending across estimated 20–25 further sessions
...
docs/                                   # Setup guides, troubleshooting, this file
.claude/
...
Note: `expert-system/output/` (generated XLSX files) is not gitignored but the directory does not exist until Phase B produces output.
